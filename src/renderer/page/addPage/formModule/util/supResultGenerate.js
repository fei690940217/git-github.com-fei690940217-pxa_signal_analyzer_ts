...
const fetchProjectConfig = async () => {
    return await ipcRenderer.invoke("getJsonFileByFilePath", "app/addProjectConfig.json");
};
//sup排序
//计算排序的数字
...
const supResultGenerate = async (formValue) => {
    try {
...
const nsaSupResultGenerate = async (formValue) => {
    try {
        ...
        const list = tempSupList.flatMap(supItem => {
            ...
            if (LTE_Band?.length) {
                const tempSupList = LTE_Band.map(let_band => {
...
export default async (formValue) => {
