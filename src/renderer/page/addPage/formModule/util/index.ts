 ...
 * @LastEditors: feifei
 * @LastEditTime: 2024-12-20 10:49:46
 * @Descripttion: 测试数据生成
 */
...
//子级数据生成
const subsHandle = async (
  list: SupRowType[],
  RBList: RBItemType[],
  formValue: AddFormValueType,
) => {
  try {
    const { testItems } = formValue;
    const supList = list;
    //获取新增项目的配置文件
    //LTE_Band_List
...
const BandEdgeHandle = async (supList: ResultItemType[], isGate: boolean) => {
  try {
