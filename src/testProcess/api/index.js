...
//获取连接列表
exports.get_instr_list = (config = {}) => {
  return new Promise(async (resolve, reject) => {
    try {
      const res = await axiosInstance.post("/get_instr", { instructName: '获取visaProxy连接列表' }, config);
      if (res.data.error === 0) {
        resolve(res.data.rst);
      } else {
        reject(res.data.errmsg);
...
exports.delete_instr = (params, config = {}) => {
  return new Promise(async (resolve, reject) => {
    try {
...
//向设备写入指令
exports.write_fn = (params, config = {}) => {
  return new Promise(async (resolve, reject) => {
    try {
 ...
 */
exports.query_and_write = async (params, config = {}) => {
  try {
 ...
 */
exports.get_screen_capture = (params, config = {}) => {
  // let params = {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const res = await axiosInstance.post("/query_screen_capture", params, config);
      if (res.data.error === 0) {
        resolve(res.data.rst);
      } else {
        reject(res.data.errmsg);
 ...
 */
exports.set_timeout = (params, config = {}) => {
  return new Promise(async (resolve, reject) => {
    try {
 ...
 */
exports.get_soft_version = (params, config = {}) => {
  // let obj = {
  //   appname: appname,
  // };
