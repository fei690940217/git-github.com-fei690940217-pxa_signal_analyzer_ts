 ...
 * @LastEditors: feifei
 * @LastEditTime: 2025-01-10 11:11:16
 * @FilePath: \pxa_signal_analyzer\src\main\ipcMain\ipcMainModule1\archiveProject.ts
...
export default async (
  ...
  try {
    ...
    //确保archive文件夹存在
    await ensureDir(archiveDirPath);
    const promiseList = subProjectNameList.map((item) => {
      const formPath = path.join(dirPath, item);
      const toPath = path.join(archiveDirPath, item);
      return moveProject(formPath, toPath);
    });
